...
public class DDOServer1 extends CORBAImplementationPOA implements Runnable {
	...
	protected DDOServer1(String serverName, int s1Port, int UDP_Port1) {
		super();
		sPort = s1Port;
		...
		try {
			...
			conSocket2 = new DatagramSocket(PortAssignment.UDPPort2DDOServer1);
			conSocket3 = new DatagramSocket(PortAssignment.UDPPort3DDOServer1);
	...
	public String getRecordCounts() {
		...
		result += " LVL :- " + UDPhandlingClient(PortAssignment.UDPPortLVLServer1,req) + " MTL :- " + UDPhandlingClient(PortAssignment.UDPPortMTLServer1,req);
		return result;
	}
	...
	public static void main(String [] args) {
		try {
			...
			DDOServer1 ddoServer1 = new DDOServer1("DDOServer1", PortAssignment.DDOServer1, PortAssignment.UDPPortDDOServer1);
			ddoServer1.setORB(objectorb);
			
			org.omg.CORBA.Object objectreference2 = cobraimplroot.servant_to_reference(ddoServer1);
			CORBAImplementation objectreference3 = CORBAImplementationHelper.narrow(objectreference2);
			
			String name = "DDOServer1";
			NameComponent nameobject[] = refncincorba.to_name(name);
			refncincorba.rebind(nameobject, objectreference3);
			System.out.println(ddoServer1.sName + " server started");
			(new Thread(ddoServer1)).start();
			new Thread(new Runnable() {

				public void run() {
					System.out.println("I am in thread of DDOServer1");
					HearBeat();
				}
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				message1.setMessage("Request received at DDOServer1 from port: "+request.getPort());
				...
				if (reqReceived.requestType == Request.CTR) {
					...
					if (createTrecordSuccess) {
						...
						System.out.println("DDOServer1:Teacher is added successfully.");
						result = "true";
					} else {
						message1.setMessage("Failed: Teacher Record has not been created.");
						System.out.println("DDOServer1:Error: Teacher is not added.");
						result = "false";
					}
					...
					if (createSrecordSucess) {
						...
						System.out.println("DDOServer1:Student is added successfully.");
						result = "true";
					} else {
						 message1.setMessage("Failed: Student Record has not been created.");
						System.out.println("DDOServer1:Error: Student is not added.");
						result = "false";
					}
					...
					if (editRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.fieldName,
						...
						System.out.println("DDOServer1:Record is successfully edited.");
						result = "true";
					} else {
						message1.setMessage("Failed: Unable to edit record" + reqReceived.recordID);
						System.out.println("DDOServer1:Record is not existed or new value is not valid");
						result = "false";
					}
				} else if (reqReceived.requestType == Request.TREQ) {
					message1.setMessage(">>>>>>>>DDOServer1: in request transfer.."+reqReceived.managerID+" "+reqReceived.recordID+" "+reqReceived.receivingServer);
					if (transferRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.receivingServer)) {
						...
						System.out.println("DDOServer1:Transfer successfull of Record:" + reqReceived.recordID + " to location"
								+ reqReceived.receivingServer);
						result = "true";
					} else {
						message1.setMessage("Transfer of Record " + reqReceived.recordID + " has been failed.");
						System.out.println("DDOServer1:Transfer unsuccessfull of Record:" + reqReceived.recordID);
						result = "false";
					}
					...
					System.out.println("DDOServer1:Records are: " + recordInfo);
				...
				message1.setMessage("Result after executing on DDOServer1 servers is "+result);
	...
	public static void HearBeat() {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				ReliableSocket relSocket = null;
				try {
					datagramSocket = new DatagramSocket(PortAssignment.UDPPortHeartBeat_DDO1);
					System.out.println("Inside try");
					relSocket = new ReliableSocket(new DatagramSocket());
					relSocket.connect(new InetSocketAddress(InetAddress.getByName("localhost"),9999));
					String message = "DDOServer1 is Alive!" + HeartBeatCounter.getInstance().getNextInteger() + "!" + PortAssignment.UDPPortHeartBeat_DDO1;
					
					System.out.println("DDO Server1: "+ message.split("!")[1]);
