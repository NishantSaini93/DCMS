...
public class LVLServer3 extends CORBAImplementationPOA implements Runnable {
	static int count ;
	String sName = "";
	int sPort = 0;
	...
	protected LVLServer3(String serverName, int s1Port, int UDP_Port1) {
		super();
		sPort = s1Port;
		...
		try {
			...
			conSocket2 = new DatagramSocket(PortAssignment.UDPPort2LVLServer3);
			conSocket3 = new DatagramSocket(PortAssignment.UDPPort3LVLServer3);
	...
	public String getRecordCounts() {
		...
		result += " DDO :- " + UDPhandlingClient(PortAssignment.UDPPortDDOServer3,req) + " MTL :- " + UDPhandlingClient(PortAssignment.UDPPortMTLServer3,req);
		return result;
	}
	...
	public static void main(String [] args) {
		try {
			...
			LVLServer3 LVLServer3 = new LVLServer3("LVLServer3", PortAssignment.LVLServer3, PortAssignment.UDPPortLVLServer3);
			LVLServer3.setORB(objectorb);
			
			org.omg.CORBA.Object objectreference2 = cobraimplroot.servant_to_reference(LVLServer3);
			CORBAImplementation objectreference3 = CORBAImplementationHelper.narrow(objectreference2);
			
			String name = "LVLServer3";
			NameComponent nameobject[] = refncincorba.to_name(name);
			refncincorba.rebind(nameobject, objectreference3);
			System.out.println(LVLServer3.sName + " server started");
			(new Thread(LVLServer3)).start();
			new Thread(new Runnable() {

				public void run() {
					System.out.println("I am in thread of LVLServer3");
					HearBeat();
				}
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				message1.setMessage("Request received at LVLServer3 from port: "+request.getPort());
				...
				if (reqReceived.requestType == Request.CTR) {
					...
					if (createTrecordSuccess) {
						...
						System.out.println("LVLServer3:Teacher is added successfully.");
						result = "true";
					} else {
						message1.setMessage("Failed: Teacher Record has not been created.");
						System.out.println("LVLServer3:Error: Teacher is not added.");
						result = "false";
					}
					...
					if (createSrecordSucess) {
						...
						System.out.println("LVLServer3:Student is added successfully.");
						result = "true";
					} else {
						 message1.setMessage("Failed: Student Record has not been created.");
						System.out.println("LVLServer3:Error: Student is not added.");
						result = "false";
					}
					...
					if (editRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.fieldName,
						...
						System.out.println("LVLServer3:Record is successfully edited.");
						result = "true";
					} else {
						message1.setMessage("Failed: Unable to edit record" + reqReceived.recordID);
						System.out.println("LVLServer3:Record is not existed or new value is not valid");
						result = "false";
					}
				} else if (reqReceived.requestType == Request.TREQ) {
					message1.setMessage(">>>>>>>>LVLServer3: in request transfer.."+reqReceived.managerID+" "+reqReceived.recordID+" "+reqReceived.receivingServer);
					if (transferRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.receivingServer)) {
						...
						System.out.println("LVLServer3:Transfer successfull of Record:" + reqReceived.recordID + " to location"
								+ reqReceived.receivingServer);
						result = "true";
					} else {
						message1.setMessage("Transfer of Record " + reqReceived.recordID + " has been failed.");
						System.out.println("LVLServer3:Transfer unsuccessfull of Record:" + reqReceived.recordID);
						result = "false";
					}
					...
					System.out.println("LVLServer3:Records are: " + recordInfo);
					System.out.println(">>>>sending to port"+request.getPort());
					result = "true";
					System.out.println("LVLServer3: string send is"+result);
				}
				//after executing on its own
				//broadcasting to other Slaves
				message1.setMessage("Result after executing on LVLServer3 servers is "+result);
				if (result.startsWith("true")) {
					...
					if (request.getPort() == PortAssignment.UDPPort_FE && reqReceived.requestType!= Request.GCREQ) {
						...
						DatagramPacket request3 = new DatagramPacket(serializedMsg, serializedMsg.length, ahost,
								PortAssignment.UDPPortLVLServer1);
						
						conSocket3.setSoTimeout(4000);
						
						try {
							message1.setMessage("******Sending to LVLServer1******");
							conSocket3.send(request3);
							relSocket.write(serializedMsg, 0, serializedMsg.length);
							message1.setMessage("******Sent to LVLServer1******");
							DatagramPacket reply3 = new DatagramPacket(buffer3, buffer3.length);
							conSocket3.receive(reply3);
							message1.setMessage("******Received reply from LVLServer1******");
							byte[] resultRecieved3 = reply3.getData();
							result3 = new String(resultRecieved3);
							message1.setMessage("Result received from LVLServer1 after Broadcast: "+result3);
						} catch (SocketTimeoutException e) {
							System.out.println("Socket has timed out to send to LVLServer1");
						}
	...
	public static void HearBeat() {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				ReliableSocket relSocket = null;
				try {
					datagramSocket = new DatagramSocket(PortAssignment.UDPPortHeartBeat_LVL3);
					System.out.println("Inside try");
					relSocket = new ReliableSocket(new DatagramSocket());
					relSocket.connect(new InetSocketAddress(InetAddress.getByName("localhost"),9999));
					String message = "LVLServer3 is Alive!" + HeartBeatCounter.getInstance().getNextInteger() + "!" + PortAssignment.UDPPortHeartBeat_LVL3;
					
					System.out.println("LVL Server3 "+ message.split("!")[1]);
