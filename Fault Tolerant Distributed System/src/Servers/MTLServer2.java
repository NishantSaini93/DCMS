...
public class MTLServer2 extends CORBAImplementationPOA implements Runnable {
	...
	protected MTLServer2(String serverName, int s1Port, int UDP_Port1) {
		super();
		sPort = s1Port;
		objectofrecord = new HashMapRecord();
		ListeningportUDP = UDP_Port1;
		this.sName = serverName;
		message1= new LogEvent("MTLServer2");
		...
		try {
			...
			conSocket2 = new DatagramSocket(PortAssignment.UDPPort2MTLServer2);
			conSocket3 = new DatagramSocket(PortAssignment.UDPPort3MTLServer2);
	...
	public String getRecordCounts() {
		...
		result += " LVL :- " + UDPhandlingClient(PortAssignment.UDPPortLVLServer2,req) + " DDO :- " + UDPhandlingClient(PortAssignment.UDPPortDDOServer2,req);
		return result;
	}
	...
	public static void main(String [] args) {
		try {
			...
			MTLServer2 MTLServer2 = new MTLServer2("MTLServer2", PortAssignment.MTLServer2, PortAssignment.UDPPortMTLServer2);
			MTLServer2.setORB(objectorb);
			
			org.omg.CORBA.Object objectreference2 = cobraimplroot.servant_to_reference(MTLServer2);
			CORBAImplementation objectreference3 = CORBAImplementationHelper.narrow(objectreference2);
			
			String name = "MTLServer2";
			NameComponent nameobject[] = refncincorba.to_name(name);
			refncincorba.rebind(nameobject, objectreference3);
			System.out.println(MTLServer2.sName + " server started");
			(new Thread(MTLServer2)).start();
			new Thread(new Runnable() {

				public void run() {
					System.out.println("I am in thread of MTLServer2");
					HearBeat();
				}
	...
	public boolean transferRecord(String managerID, String recordID, String remoteCenterServerName) {
		...
		if (alreadypresentR == null) {
			...
			if (remoteCenterServerName.equalsIgnoreCase("DDO")) {
				...
				if (this.UDPhandlingClient(PortAssignment.UDPPortDDOServer2, req).startsWith("true")) {
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				message1.setMessage("Request received at MTLServer2 from port: "+request.getPort());
				...
				if (reqReceived.requestType == Request.CTR) {
					...
					if (createTrecordSuccess) {
						...
						System.out.println("MTLServer2:Teacher is added successfully.");
						result = "true";
					} else {
						message1.setMessage("Failed: Teacher Record has not been created.");
						System.out.println("MTLServer2:Error: Teacher is not added.");
						result = "false";
					}
					...
					if (createSrecordSucess) {
						...
						System.out.println("MTLServer2:Student is added successfully.");
						result = "true";
					} else {
						 message1.setMessage("Failed: Student Record has not been created.");
						System.out.println("MTLServer2:Error: Student is not added.");
						result = "false";
					}
					...
					if (editRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.fieldName,
						...
						System.out.println("MTLServer2:Record is successfully edited.");
						result = "true";
					} else {
						message1.setMessage("Failed: Unable to edit record" + reqReceived.recordID);
						System.out.println("MTLServer2:Record is not existed or new value is not valid");
						result = "false";
					}
				} else if (reqReceived.requestType == Request.TREQ) {
					message1.setMessage(">>>>>>>>MTLServer2: in request transfer.."+reqReceived.managerID+" "+reqReceived.recordID+" "+reqReceived.receivingServer);
					if (transferRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.receivingServer)) {
						...
						System.out.println("MTLServer2:Transfer successfull of Record:" + reqReceived.recordID + " to location"
								+ reqReceived.receivingServer);
						result = "true";
					} else {
						message1.setMessage("Transfer of Record " + reqReceived.recordID + " has been failed.");
						System.out.println("MTLServer2:Transfer unsuccessfull of Record:" + reqReceived.recordID);
						result = "false";
					}
					...
					System.out.println("MTLServer2:Records are: " + recordInfo);
					System.out.println(">>>>sending to port"+request.getPort());
					result = "true";
					System.out.println("MTLServer2: string send is"+result);
				}
				//after executing on its own
				//broadcasting to other Slaves
				message1.setMessage("Result after executing on MTLServer2 servers is "+result);
				if (result.startsWith("true")) {
					...
					if (request.getPort() == PortAssignment.UDPPort_FE && reqReceived.requestType!= Request.GCREQ) {
						...
						try {
							...
							message1.setMessage("Result received from MTLServer1 after Broadcast: "+result2);
						...
						DatagramPacket request3 = new DatagramPacket(serializedMsg, serializedMsg.length, ahost,
								PortAssignment.UDPPortMTLServer3);
						
						conSocket3.setSoTimeout(4000);
						
						try {
							message1.setMessage("******Sending to MTLServer3******");
							conSocket3.send(request3);
							relSocket.write(serializedMsg, 0, serializedMsg.length);
							message1.setMessage("******Sent to MTLServer3******");
							DatagramPacket reply3 = new DatagramPacket(buffer3, buffer3.length);
							conSocket3.receive(reply3);
							message1.setMessage("******Received reply from MTLServer3******");
							byte[] resultRecieved3 = reply3.getData();
							result3 = new String(resultRecieved3);
							message1.setMessage("Result received from MTLServer3 after Broadcast: "+result3);
						} catch (SocketTimeoutException e) {
							System.out.println("Socket has timed out to send to MTLServer3");
						}
	...
	public static void HearBeat() {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				ReliableSocket relSocket = null;
				try {
					datagramSocket = new DatagramSocket(PortAssignment.UDPPortHeartBeat_MTL2);
					System.out.println("Inside try");
					relSocket = new ReliableSocket(new DatagramSocket());
					relSocket.connect(new InetSocketAddress(InetAddress.getByName("localhost"),9999));
					String message = "MTLServer2 is Alive!" + HeartBeatCounter.getInstance().getNextInteger() + "!" + PortAssignment.UDPPortHeartBeat_MTL2;
					
					System.out.println("MTL Server2 "+ message.split("!")[1]);
