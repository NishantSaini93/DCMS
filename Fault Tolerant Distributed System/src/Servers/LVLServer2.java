...
public class LVLServer2 extends CORBAImplementationPOA implements Runnable {
	static int count ;
	String sName = "";
	int sPort = 0;
	...
	protected LVLServer2(String serverName, int s1Port, int UDP_Port1) {
		super();
		sPort = s1Port;
		...
		try {
			...
			conSocket2 = new DatagramSocket(PortAssignment.UDPPort2LVLServer2);
			conSocket3 = new DatagramSocket(PortAssignment.UDPPort3LVLServer2);
	...
	public String getRecordCounts() {
		...
		result += " DDO :- " + UDPhandlingClient(PortAssignment.UDPPortDDOServer2,req) + " MTL :- " + UDPhandlingClient(PortAssignment.UDPPortMTLServer2,req);
		return result;
	}
	...
	public static void main(String [] args) {
		try {
			...
			LVLServer2 LVLServer2 = new LVLServer2("LVLServer2", PortAssignment.LVLServer2, PortAssignment.UDPPortLVLServer2);
			LVLServer2.setORB(objectorb);
			
			org.omg.CORBA.Object objectreference2 = cobraimplroot.servant_to_reference(LVLServer2);
			CORBAImplementation objectreference3 = CORBAImplementationHelper.narrow(objectreference2);
			
			String name = "LVLServer2";
			NameComponent nameobject[] = refncincorba.to_name(name);
			refncincorba.rebind(nameobject, objectreference3);
			System.out.println(LVLServer2.sName + " server started");
			(new Thread(LVLServer2)).start();
			new Thread(new Runnable() {

				public void run() {
					System.out.println("I am in thread of LVLServer2");
					HearBeat();
				}
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				message1.setMessage("Request received at LVLServer2 from port: "+request.getPort());
				...
				if (reqReceived.requestType == Request.CTR) {
					...
					if (createTrecordSuccess) {
						...
						System.out.println("LVLServer2:Teacher is added successfully.");
						result = "true";
					} else {
						message1.setMessage("Failed: Teacher Record has not been created.");
						System.out.println("LVLServer2:Error: Teacher is not added.");
						result = "false";
					}
					...
					if (createSrecordSucess) {
						...
						System.out.println("LVLServer2:Student is added successfully.");
						result = "true";
					} else {
						 message1.setMessage("Failed: Student Record has not been created.");
						System.out.println("LVLServer2:Error: Student is not added.");
						result = "false";
					}
					...
					if (editRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.fieldName,
						...
						System.out.println("LVLServer2:Record is successfully edited.");
						result = "true";
					} else {
						message1.setMessage("Failed: Unable to edit record" + reqReceived.recordID);
						System.out.println("LVLServer2:Record is not existed or new value is not valid");
						result = "false";
					}
				} else if (reqReceived.requestType == Request.TREQ) {
					message1.setMessage(">>>>>>>>LVLServer2: in request transfer.."+reqReceived.managerID+" "+reqReceived.recordID+" "+reqReceived.receivingServer);
					if (transferRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.receivingServer)) {
						...
						System.out.println("LVLServer2:Transfer successfull of Record:" + reqReceived.recordID + " to location"
								+ reqReceived.receivingServer);
						result = "true";
					} else {
						message1.setMessage("Transfer of Record " + reqReceived.recordID + " has been failed.");
						System.out.println("LVLServer2:Transfer unsuccessfull of Record:" + reqReceived.recordID);
						result = "false";
					}
					...
					System.out.println("LVLServer2:Records are: " + recordInfo);
					System.out.println(">>>>sending to port"+request.getPort());
					result = "true";
					System.out.println("LVLServer2: string send is"+result);
				}
				//after executing on its own
				//broadcasting to other Slaves
				message1.setMessage("Result after executing on LVLServer2 servers is "+result);
				if (result.startsWith("true")) {
					...
					if (request.getPort() == PortAssignment.UDPPort_FE && reqReceived.requestType!= Request.GCREQ) {
						...
						try {
							...
							message1.setMessage("Result received from LVLServer1 after Broadcast: "+result2);
	...
	public static void HearBeat() {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				ReliableSocket relSocket = null;
				try {
					datagramSocket = new DatagramSocket(PortAssignment.UDPPortHeartBeat_LVL2);
					System.out.println("Inside try");
					relSocket = new ReliableSocket(new DatagramSocket());
					relSocket.connect(new InetSocketAddress(InetAddress.getByName("localhost"),9999));
					String message = "LVLServer2 is Alive!" + HeartBeatCounter.getInstance().getNextInteger() + "!" + PortAssignment.UDPPortHeartBeat_LVL2;
					
					System.out.println("LVL Server2 id "+ message.split("!")[1]);
