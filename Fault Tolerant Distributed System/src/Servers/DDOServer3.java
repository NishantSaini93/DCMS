...
public class DDOServer3 extends CORBAImplementationPOA implements Runnable {
	...
	protected DDOServer3(String serverName, int s1Port, int UDP_Port1) {
		super();
		sPort = s1Port;
		...
		try {
			...
			conSocket2 = new DatagramSocket(PortAssignment.UDPPort2DDOServer3);
			conSocket3 = new DatagramSocket(PortAssignment.UDPPort3DDOServer3);
	...
	public static void main(String [] args) {
		try {
			...
			DDOServer3 DDOServer3 = new DDOServer3("DDOServer3", PortAssignment.DDOServer3, PortAssignment.UDPPortDDOServer3);
			DDOServer3.setORB(objectorb);
			
			org.omg.CORBA.Object objectreference2 = cobraimplroot.servant_to_reference(DDOServer3);
			CORBAImplementation objectreference3 = CORBAImplementationHelper.narrow(objectreference2);
			
			String name = "DDOServer3";
			NameComponent nameobject[] = refncincorba.to_name(name);
			refncincorba.rebind(nameobject, objectreference3);
			System.out.println(DDOServer3.sName + " server started");
			(new Thread(DDOServer3)).start();
			new Thread(new Runnable() {

				public void run() {
					System.out.println("I am in thread of DDOServer3");
					HearBeat();
				}
	...
	public boolean transferRecord(String managerID, String recordID, String remoteCenterServerName) {
		...
		if (alreadypresentR == null) {
			...
			if (remoteCenterServerName.equalsIgnoreCase("MTL")) {
				...
				if (this.UDPhandlingClient(PortAssignment.UDPPortMTLServer3, req).startsWith("true")) {
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				message1.setMessage("Request received at DDOServer3 from port: "+request.getPort());
				...
				if (reqReceived.requestType == Request.CTR) {
					...
					if (createTrecordSuccess) {
						...
						System.out.println("DDOServer3:Teacher is added successfully.");
						result = "true";
					} else {
						message1.setMessage("Failed: Teacher Record has not been created.");
						System.out.println("DDOServer3:Error: Teacher is not added.");
						result = "false";
					}
					...
					if (createSrecordSucess) {
						...
						System.out.println("DDOServer3:Student is added successfully.");
						result = "true";
					} else {
						 message1.setMessage("Failed: Student Record has not been created.");
						System.out.println("DDOServer3:Error: Student is not added.");
						result = "false";
					}
					...
					if (editRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.fieldName,
						...
						System.out.println("DDOServer3:Record is successfully edited.");
						result = "true";
					} else {
						message1.setMessage("Failed: Unable to edit record" + reqReceived.recordID);
						System.out.println("DDOServer3:Record is not existed or new value is not valid");
						result = "false";
					}
				} else if (reqReceived.requestType == Request.TREQ) {
					message1.setMessage(">>>>>>>>DDOServer3: in request transfer.."+reqReceived.managerID+" "+reqReceived.recordID+" "+reqReceived.receivingServer);
					if (transferRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.receivingServer)) {
						...
						System.out.println("DDOServer3:Transfer successfull of Record:" + reqReceived.recordID + " to location"
								+ reqReceived.receivingServer);
						result = "true";
					} else {
						message1.setMessage("Transfer of Record " + reqReceived.recordID + " has been failed.");
						System.out.println("DDOServer3:Transfer unsuccessfull of Record:" + reqReceived.recordID);
						result = "false";
					}
					...
					System.out.println("DDOServer3:Records are: " + recordInfo);
					System.out.println(">>>>sending to port"+request.getPort());
					result = "true";
					System.out.println("DDOServer3: string send is"+result);
				}
				//after executing on its own
				//broadcasting to other Slaves
				message1.setMessage("Result after executing on DDOServer3 servers is "+result);
				if (result.startsWith("true")) {
					...
					if (request.getPort() == PortAssignment.UDPPort_FE && reqReceived.requestType!= Request.GCREQ) {
						...
						DatagramPacket request2 = new DatagramPacket(serializedMsg, serializedMsg.length, ahost,
								PortAssignment.UDPPortDDOServer2);
						
						conSocket2.setSoTimeout(4000);
						
						try {
							message1.setMessage("******Sending to DDOServer2******");
							conSocket2.send(request2);
							relSocket.write(serializedMsg, 0, serializedMsg.length);
							message1.setMessage("******Sent to DDOServer2******");
							DatagramPacket reply2 = new DatagramPacket(buffer2, buffer2.length);
							conSocket2.receive(reply2);
							message1.setMessage("******Received reply from DDOServer2******");
							byte[] resultRecieved2 = reply2.getData();
							result2 = new String(resultRecieved2);
							message1.setMessage("Result received from DDOServer2 after Broadcast: "+result2);
						} catch (SocketTimeoutException e) {
							System.out.println("Socket has timed out to send to DDOServer2");
						}
						...
						try {
							...
							message1.setMessage("Result received from DDOServer1 after Broadcast: "+result3);
						} catch (SocketTimeoutException e) {
				...
				System.out.println("result.getbytes().length=" + result.getBytes().length);
	...
	public static void HearBeat() {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				try {
					datagramSocket = new DatagramSocket(PortAssignment.UDPPortHeartBeat_DDO3);
					System.out.println("Inside try");
					relSocket = new ReliableSocket(new DatagramSocket());
					relSocket.connect(new InetSocketAddress(InetAddress.getByName("localhost"),9999));
					String message = "DDOServer3 is Alive!" + HeartBeatCounter.getInstance().getNextInteger() + "!" + PortAssignment.UDPPortHeartBeat_DDO3;
					System.out.println("DDO Server3 "+ message.split("!")[1]);
