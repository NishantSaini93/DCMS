...
public class MTLServer3 extends CORBAImplementationPOA implements Runnable {
	...
	protected MTLServer3(String serverName, int s1Port, int UDP_Port1) {
		super();
		sPort = s1Port;
		objectofrecord = new HashMapRecord();
		ListeningportUDP = UDP_Port1;
		this.sName = serverName;
		message1= new LogEvent("MTLServer3");
		...
		try {
			...
			conSocket2 = new DatagramSocket(PortAssignment.UDPPort2MTLServer3);
			conSocket3 = new DatagramSocket(PortAssignment.UDPPort3MTLServer3);
	...
	public String getRecordCounts() {
		...
		String result = "MTL" + " :- " + objectofrecord.fetchRecordCount();
		Request req = new Request();
		req.requestType = Request.GMYCREQ;
		result += " LVL :- " + UDPhandlingClient(PortAssignment.UDPPortLVLServer3,req) + " DDO :- " + UDPhandlingClient(PortAssignment.UDPPortDDOServer3,req);
		return result;
	}
	...
	public static void main(String [] args) {
		try {
			...
			MTLServer3 MTLServer3 = new MTLServer3("MTLServer3", PortAssignment.MTLServer3, PortAssignment.UDPPortMTLServer3);
			MTLServer3.setORB(objectorb);
			
			org.omg.CORBA.Object objectreference2 = cobraimplroot.servant_to_reference(MTLServer3);
			CORBAImplementation objectreference3 = CORBAImplementationHelper.narrow(objectreference2);
			
			String name = "MTLServer3";
			NameComponent nameobject[] = refncincorba.to_name(name);
			refncincorba.rebind(nameobject, objectreference3);
			System.out.println(MTLServer3.sName + " server started");
			(new Thread(MTLServer3)).start();
			new Thread(new Runnable() {

				public void run() {
					System.out.println("I am in thread of MTLServer3");
					HearBeat();
				}
	...
	public boolean transferRecord(String managerID, String recordID, String remoteCenterServerName) {
		...
		if (alreadypresentR == null) {
			...
			if (remoteCenterServerName.equalsIgnoreCase("LVL")) {
				...
				if (this.UDPhandlingClient(PortAssignment.UDPPortLVLServer3, req).startsWith("true")) {
			...
			if (remoteCenterServerName.equalsIgnoreCase("DDO")) {
				...
				if (this.UDPhandlingClient(PortAssignment.UDPPortDDOServer3, req).startsWith("true")) {
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				message1.setMessage("Request received at MTLServer3 from port: "+request.getPort());
				...
				if (reqReceived.requestType == Request.CTR) {
					...
					if (createTrecordSuccess) {
						...
						System.out.println("MTLServer3:Teacher is added successfully.");
						result = "true";
					} else {
						message1.setMessage("Failed: Teacher Record has not been created.");
						System.out.println("MTLServer3:Error: Teacher is not added.");
						result = "false";
					}
					...
					if (createSrecordSucess) {
						...
						System.out.println("MTLServer3:Student is added successfully.");
						result = "true";
					} else {
						 message1.setMessage("Failed: Student Record has not been created.");
						System.out.println("MTLServer3:Error: Student is not added.");
						result = "false";
					}
					...
					if (editRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.fieldName,
						...
						System.out.println("MTLServer3:Record is successfully edited.");
						result = "true";
					} else {
						message1.setMessage("Failed: Unable to edit record" + reqReceived.recordID);
						System.out.println("MTLServer3:Record is not existed or new value is not valid");
						result = "false";
					}
				} else if (reqReceived.requestType == Request.TREQ) {
					message1.setMessage(">>>>>>>>MTLServer3: in request transfer.."+reqReceived.managerID+" "+reqReceived.recordID+" "+reqReceived.receivingServer);
					if (transferRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.receivingServer)) {
						...
						System.out.println("MTLServer3:Transfer successfull of Record:" + reqReceived.recordID + " to location"
								+ reqReceived.receivingServer);
						result = "true";
					} else {
						message1.setMessage("Transfer of Record " + reqReceived.recordID + " has been failed.");
						System.out.println("MTLServer3:Transfer unsuccessfull of Record:" + reqReceived.recordID);
						result = "false";
					}
					...
					System.out.println("MTLServer3:Records are: " + recordInfo);
					System.out.println(">>>>sending to port"+request.getPort());
					result = "true";
					System.out.println("MTLServer3: string send is"+result);
				}
				//after executing on its own
				//broadcasting to other Slaves
				message1.setMessage("Result after executing on MTLServer1 servers is "+result);
				if (result.startsWith("true")) {
					...
					if (request.getPort() == PortAssignment.UDPPort_FE && reqReceived.requestType!= Request.GCREQ) {
						...
						DatagramPacket request2 = new DatagramPacket(serializedMsg, serializedMsg.length, ahost,
								PortAssignment.UDPPortMTLServer2);
						
						conSocket2.setSoTimeout(4000);
						
						try {
							message1.setMessage("******Sending to MTLServer2******");
							conSocket2.send(request2);
							relSocket.write(serializedMsg, 0, serializedMsg.length);
							message1.setMessage("******Sent to MTLServer2******");
							DatagramPacket reply2 = new DatagramPacket(buffer2, buffer2.length);
							conSocket2.receive(reply2);
							message1.setMessage("******Received reply from MTLServer2******");
							byte[] resultRecieved2 = reply2.getData();
							result2 = new String(resultRecieved2);
							message1.setMessage("Result received from MTLServer2 after Broadcast: "+result2);
						} catch (SocketTimeoutException e) {
							System.out.println("Socket has timed out to send to MTLServer2");
						}
						
						DatagramPacket request3 = new DatagramPacket(serializedMsg, serializedMsg.length, ahost,
								PortAssignment.UDPPortMTLServer1);
						
						conSocket3.setSoTimeout(4000);
						
						try {
							message1.setMessage("******Sending to MTLServer1******");
							conSocket3.send(request3);
							relSocket.write(serializedMsg, 0, serializedMsg.length);
							message1.setMessage("******Sent to MTLServer1******");
							DatagramPacket reply3 = new DatagramPacket(buffer3, buffer3.length);
							conSocket3.receive(reply3);
							message1.setMessage("******Received reply from MTLServer1******");
							byte[] resultRecieved3 = reply3.getData();
							result3 = new String(resultRecieved3);
							message1.setMessage("Result received from MTLServer1 after Broadcast: "+result3);
						} catch (SocketTimeoutException e) {
							System.out.println("Socket has timed out to send to MTLServer1");
						}
	...
	public static void HearBeat() {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				ReliableSocket relSocket = null;
				try {
					datagramSocket = new DatagramSocket(PortAssignment.UDPPortHeartBeat_MTL3);
					System.out.println("Inside try");
					relSocket = new ReliableSocket(new DatagramSocket());
					relSocket.connect(new InetSocketAddress(InetAddress.getByName("localhost"),9999));
					String message = "MTLServer3 is Alive!" + HeartBeatCounter.getInstance().getNextInteger() + "!" + PortAssignment.UDPPortHeartBeat_MTL3;
					
					System.out.println("MTL Server3 "+ message.split("!")[1]);
