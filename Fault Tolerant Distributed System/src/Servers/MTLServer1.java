...
public class MTLServer1 extends CORBAImplementationPOA implements Runnable {
	...
	protected MTLServer1(String serverName, int s1Port, int UDP_Port1) {
		super();
		sPort = s1Port;
		objectofrecord = new HashMapRecord();
		ListeningportUDP = UDP_Port1;
		this.sName = serverName;
		message1= new LogEvent("MTLServer1");
		...
		try {
			...
			conSocket2 = new DatagramSocket(PortAssignment.UDPPort2MTLServer1);
			conSocket3 = new DatagramSocket(PortAssignment.UDPPort3MTLServer1);
	...
	public String getRecordCounts() {
		...
		result += " LVL :- " + UDPhandlingClient(PortAssignment.UDPPortLVLServer1,req) + " DDO :- " + UDPhandlingClient(PortAssignment.UDPPortDDOServer1,req);
		return result;
	}
	...
	public static void main(String [] args) {
		try {
			...
			MTLServer1 mtlServer1 = new MTLServer1("MTLServer1", PortAssignment.MTLServer1, PortAssignment.UDPPortMTLServer1);
			mtlServer1.setORB(objectorb);
			
			org.omg.CORBA.Object objectreference2 = cobraimplroot.servant_to_reference(mtlServer1);
			CORBAImplementation objectreference3 = CORBAImplementationHelper.narrow(objectreference2);
			
			String name = "MTLServer1";
			NameComponent nameobject[] = refncincorba.to_name(name);
			refncincorba.rebind(nameobject, objectreference3);
			System.out.println(mtlServer1.sName + " server started");
			(new Thread(mtlServer1)).start();
			new Thread(new Runnable() {

				public void run() {
					System.out.println("I am in thread of MTLServer1");
					HearBeat();
				}
	...
	public boolean transferRecord(String managerID, String recordID, String remoteCenterServerName) {
		...
		if (alreadypresentR == null) {
			...
			if (remoteCenterServerName.equalsIgnoreCase("LVL")) {
				...
				if (this.UDPhandlingClient(PortAssignment.UDPPortLVLServer1, req).startsWith("true")) {
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				message1.setMessage("Request received at MTLServer1 from port: "+request.getPort());
				...
				if (reqReceived.requestType == Request.CTR) {
					...
					if (createTrecordSuccess) {
						...
						System.out.println("MTLServer1:Teacher is added successfully.");
						result = "true";
					} else {
						message1.setMessage("Failed: Teacher Record has not been created.");
						System.out.println("MTLServer1:Error: Teacher is not added.");
						result = "false";
					}
					...
					if (createSrecordSucess) {
						...
						System.out.println("MTLServer1:Student is added successfully.");
						result = "true";
					} else {
						 message1.setMessage("Failed: Student Record has not been created.");
						System.out.println("MTLServer1:Error: Student is not added.");
						result = "false";
					}
					...
					if (editRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.fieldName,
						...
						System.out.println("MTLServer1:Record is successfully edited.");
						result = "true";
					} else {
						message1.setMessage("Failed: Unable to edit record" + reqReceived.recordID);
						System.out.println("MTLServer1:Record is not existed or new value is not valid");
						result = "false";
					}
				} else if (reqReceived.requestType == Request.TREQ) {
					message1.setMessage(">>>>>>>>MTLServer1: in request transfer.."+reqReceived.managerID+" "+reqReceived.recordID+" "+reqReceived.receivingServer);
					if (transferRecord(reqReceived.managerID, reqReceived.recordID, reqReceived.receivingServer)) {
						...
						System.out.println("MTLServer1:Transfer successfull of Record:" + reqReceived.recordID + " to location"
								+ reqReceived.receivingServer);
						result = "true";
					} else {
						message1.setMessage("Transfer of Record " + reqReceived.recordID + " has been failed.");
						System.out.println("MTLServer1:Transfer unsuccessfull of Record:" + reqReceived.recordID);
						result = "false";
					}
					...
					System.out.println("MTLServer1:Records are: " + recordInfo);
					System.out.println(">>>>sending to port"+request.getPort());
					result = "true";
					System.out.println("MTLServer1: string send is"+result);
				}
	...
	public static void HearBeat() {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				ReliableSocket relSocket = null;
				try {
					datagramSocket = new DatagramSocket(PortAssignment.UDPPortHeartBeat_MTL1);
					System.out.println("Inside try");
					relSocket = new ReliableSocket(new DatagramSocket());
					relSocket.connect(new InetSocketAddress(InetAddress.getByName("localhost"),9999));
					String message = "MTLServer1 is Alive!" + HeartBeatCounter.getInstance().getNextInteger() + "!" + PortAssignment.UDPPortHeartBeat_MTL1;
					
					System.out.println("MTL Server1 "+ message.split("!")[1]);
