...
public class ManagerClient {
	...
	static FrontEndImplementation server;
	...
	public static void main(String[] args) throws ParseException, Exception {
		...
		FrontEndImplementation server = null;
		...
		while (true) {
			...
			if (ManagerClient.isManager(managerID)) {
				...
				try {
					...
					server = FrontEndImplementationHelper.narrow(ncRef
							.resolve_str(name));
					...
					while (valid) {
						...
						case 1:
							...
							boolean createSrecordSucess = server.createSRecord(managerID, fName, lName, coursesRegistered,
									status,  statusDate);
						...
						case 2:
							...
							boolean createTrecordSuccess = server.createTRecord(managerID, fName, lName, pAddress, phone,
						...
						case 4:
							...
							do{
								...
								if (checkInputs(fieldName, newValue,sName)) {
									...
									if (server.editRecord(managerID, recordID, fieldName, newValue)) {
						...
						case 5:// transfer Record
							...
							} while (!ManagerClient.checkRecordId(recordTransfer));
							do {
								System.out
										.println("Enter Location you want to transfer record:");
								transferLoc = transferSc.nextLine();
							} while (!ManagerClient.checkCity(transferLoc)
									|| transferLoc.equals(sName));

							if (server.transferRecord(managerID,
									recordTransfer, transferLoc)) {
								message1.setMessage("Record ID: "
										+ recordTransfer
								...
								message1.setMessage("Transfer of Record "
	...
	public static void displayMenu() {
		System.out.println("Please Select one of below options (1-6):");
